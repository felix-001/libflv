// Last Update:2019-06-04 16:23:29
/**
 * @file flv.c
 * @brief flv enc
 * @author qiniu
...
static int FLVWriteAudioConfig( FLVContext *ctx, FrameInfo *frame )
{
    ...
    if (idx < 0) {
        LinkLogError("findFrequencyIndex error");
	    return -1;
    }
    c[0] |= 0x10;
    c[0] |= (idx>>1);
    ...
    if ( ctx->audiocodec == FLV_CODECID_AAC ) {
        if ( isconfig ) {
            ...
            if ( ret != ADTS_DECODE_OK ) {
                LinkLogError("adts decode fail, ret = %d", ret );
                free( psave );
                goto err;
            }
...
int FLVWriteAVData( FLVContext *ctx, FrameInfo *frame )
{
    ...
    if ( frame->type == TAG_TYPE_VIDEO ) {
        ...
        if (  ctx->audiocodec == FLV_CODECID_AAC 
              ...
              && (((frame->data[1] >> 4) & 0x0F) == 0x0F)) ) {
            LinkLogError("audio data is not aac, but param set aac\n");
            goto err;
        }
